...
2. **The priority must map to a channel that includes email.** `get_delivery_channels()` gives URGENT `['in_app','email','sms']`, MEDIUM `['in_app','email']`, and **HIGH only `['in_app','sms']`** — and SMS is dark until the toll-free number clears. A HIGH template needs `channels_override` or it cannot email at all.
