...
**Review Request System**: After a repair completes, `review_request_hook` (`apps/technician_portal/hooks.py`) calls `ReviewRequestService.schedule_review_request`, which queues a Google-review request (per-tenant `ReviewConfig`, Settings → Reviews tab). The `send_review_requests` command (EB cron `12_reviews_cron.config`, every 20 min) sends due requests — concurrency-safe via `select_for_update(skip_locked=True)` (CODE-230). **Channels**: email (`ReviewConfig.send_via_email`, default on) and/or SMS via AWS SNS (`send_via_sms`, default off; phone normalized to E.164, message kept to one 160-char segment with the tracked link intact). Contact resolution falls back to `Customer.email`/`Customer.phone` when there's no primary-contact `CustomerUser` — so retail/walk-in customers captured on the repair form get requests too; `ReviewRequest.sent_via` records the channel(s). Opt-out is honored at both levels (`CustomerUser.review_opt_out`, `Customer.review_opt_out`). **Fleet accounts are excluded by default** (`ReviewConfig.send_to_fleet=False`, skip_reason `fleet_disabled`) — only RETAIL/WALK_IN customers get requests unless the shop enables the "Include Fleet Accounts" toggle. Note `Customer.customer_type` defaults to `'FLEET'`. Tests that exercise sending must set `send_to_fleet=True` or use a RETAIL customer. See `docs/proposals/review-request-system.md`.
