...
def _award(customer, amount, tenant=None, txn_type='repair_complete'):
    return LoyaltyService.award_points(
        customer, amount, txn_type, f'Test award {amount}', tenant=tenant,
    )
...
class ReconcileBalanceServiceTest(TestCase):
    ...
    def test_in_sync_when_ledger_matches_cache(self):
        _award(self.customer, 100)
        result = LoyaltyService.reconcile_balance(self.customer)
    ...
    def test_reconcile_does_not_mutate(self):
        _award(self.customer, 100)
        reward_before = Reward.objects.get(customer=self.customer).points
        LoyaltyService.reconcile_balance(self.customer)
        reward_after = Reward.objects.get(customer=self.customer).points
    ...
    def test_multiple_transactions_summed(self):
        _award(self.customer, 50)
        _award(self.customer, 25)
        _award(self.customer, 75)
        result = LoyaltyService.reconcile_balance(self.customer)
        self.assertTrue(result['in_sync'])
...
class ManualAdjustmentServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.customer = self.cu.customer
        _award(self.customer, 200)  # starting balance of 200
    ...
    def test_tenant_resolved_from_customer_if_not_passed(self):
        """manual_adjustment resolves tenant from customer.tenant."""
        pt = LoyaltyService.manual_adjustment(
            customer=self.customer,
            amount=10,
...
class PointLiabilityReportServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.customer1 = self.cu1.customer
        self.customer2 = self.cu2.customer
    ...
    def test_outstanding_points_sums_across_customers(self):
        _award(self.customer1, 100)
        _award(self.customer2, 50)
        report = LoyaltyService.get_point_liability_report(self.tenant)
    ...
    def test_redeemed_points_subtracted_from_liability(self):
        _award(self.customer1, 200)
        # Simulate redemption (negative transaction)
        _award(self.customer1, -50, txn_type='redemption')
        report = LoyaltyService.get_point_liability_report(self.tenant)
        self.assertEqual(report['total_outstanding_points'], 150)
    ...
    def test_expired_points_tracked_separately(self):
        _award(self.customer1, 200)
        _award(self.customer1, -30, txn_type='expiration')
        report = LoyaltyService.get_point_liability_report(self.tenant)
    ...
    def test_manual_adjustments_tracked(self):
        _award(self.customer1, 100)
        LoyaltyService.manual_adjustment(self.customer1, 25, 'Bonus', self.owner)
        report = LoyaltyService.get_point_liability_report(self.tenant)
    ...
    def test_active_customer_count_correct(self):
        _award(self.customer1, 100)
        # customer2 has no points — should not count
        report = LoyaltyService.get_point_liability_report(self.tenant)
    ...
    def test_customer_list_contains_correct_data(self):
        ...
        self.assertIn(self.customer1.email, emails)
    ...
    def test_tenant_isolation_in_report(self):
        ...
        _award(other_cu.customer, 9999, tenant=other_tenant)
...
class ReconcileCommandTest(TestCase):
    ...
    def test_reports_drift_without_fixing(self):
        _award(self.customer, 100)
        # Corrupt balance
        Reward.objects.filter(customer=self.customer).update(points=999)
        output = self._call_command()
    ...
    def test_tenant_id_filter_limits_scope(self):
        ...
        _award(other_cu.customer, 50, tenant=other_tenant)
        Reward.objects.filter(customer=other_cu.customer).update(points=9999)

        # Reconcile only self.tenant — other_tenant's drift should NOT be fixed
        self._call_command('--fix', tenant_id=self.tenant.pk)
        other_reward = Reward.objects.get(customer=other_cu.customer)
    ...
    def test_missing_reward_row_logged(self):
        """Customer with transactions but no Reward row is flagged."""
        ...
        Reward.objects.filter(customer=self.customer).delete()
        output = self._call_command('--json')
        data = json.loads(output)
...
class ExpireLoyaltyCommandTest(TestCase):
    ...
    def _create_expired_transaction(self, amount=100):
        ...
        pt = _award(self.customer, amount)
    ...
    def test_tenant_id_filter(self):
        ...
        other_pt = _award(other_cu.customer, 200, tenant=other_tenant)
        past = timezone.now() - timedelta(days=1)
    ...
    def test_only_positive_transactions_expire(self):
        ...
        reward = Reward.objects.create(
            tenant=self.tenant, customer=self.customer, points=0,
        )
        past = timezone.now() - timedelta(days=1)
        PointTransaction.objects.create(
            tenant=self.tenant,
    ...
    def test_balance_clamped_to_zero_if_cache_wrong(self):
        ...
        Reward.objects.filter(customer=self.customer).update(points=50)
        self._call_command()
        reward = Reward.objects.get(customer=self.customer)
...
class ManualAdjustmentViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.customer = self.cu.customer
        _award(self.customer, 200)
        self.url = reverse('owner_loyalty_adjust_points', kwargs={'customer_id': self.customer.pk})
    ...
    def test_cross_tenant_returns_404(self):
        ...
        _award(other_cu.customer, 100, tenant=other_tenant)

        self.client.force_login(self.owner)
        cross_url = reverse('owner_loyalty_adjust_points', kwargs={'customer_id': other_cu.customer.pk})
    ...
    def test_creates_point_transaction_in_db(self):
        self.client.force_login(self.owner)
        before = PointTransaction.objects.filter(customer=self.customer).count()
        self.client.post(self.url, {'amount': '50', 'reason': 'DB check'})
        after = PointTransaction.objects.filter(customer=self.customer).count()
