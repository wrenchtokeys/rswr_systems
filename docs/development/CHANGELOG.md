...
### Changed
- **The hero's hand-built HTML dashboard mock is gone.** The hero and a new "This is the
  actual product" section show captures of the real owner dashboard, jobs list, New Job form
  and customer portal. They are regenerated by `python scripts/landing_shots.py` (seeds a
  fictional shop into a throwaway SQLite DB, starts the dev server, drives headless Chrome
  over the DevTools protocol, writes `static/images/landing/*.webp`).
- **The four-stat trust bar is out** ("500+ Jobs Tracked · $50K+ Invoiced · 100% Mobile
  Friendly · 24/7 Access Anywhere"). In its place: four checkable claims and the founder's
  note, moved from below the features to directly under the hero and labelled as such.
- **New switching section**: how a shop moves from paper, spreadsheets or another system
  with what exists today, plus the contact form for help with a customer list.
- Copy: "Email and text notifications" → email only (SMS is not live); "See why shop owners
  love it" → "No credit card, no contract"; the JSON-LD `AggregateOffer` price range now
  derives from the plan query instead of a hardcoded 0–249.

### Added
- `manage.py seed_demo_shop [--reset]` — the fictional Clearview Auto Glass tenant for
  screenshots and manual QA. DEBUG-only, deterministic, dates relative to today.
- `tests/test_landing_credibility.py` — no filler statistics, no mock tells, every capture
  exists and is referenced with dimensions + alt text, founder note precedes features,
  switching section present, structured data matches the plan cards.
