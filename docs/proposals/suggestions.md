...
**~~The blocking dependency on SendGrid is a real problem.~~** *Resolved 2026-07-09:* the app migrated to Amazon SES (production access, 50,000 msg/day), so email sending is no longer a blocker for this proposal or the AI Email Template Assistant. The core infrastructure change (switch from `EmailMessage` to `EmailMultiAlternatives`) remains worth doing as a separate task.
