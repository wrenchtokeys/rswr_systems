...
**Last updated:** 2026-09-07 (C1 deployed 00:59 UTC; steps 1–4 of §What happens next are closed and on prod, step 5 waits on the sign-off line)
...
5. **The three spine features, one session each**, after §The decision carries a name.
   **This is now the head of the queue and it is blocked on the sign-off line, not on
   code.** Nothing else in this list is a session. One owner task sits beside it: read
   the live landing copy once (the switching section promises "send your customer list
   through the contact form" — a manual import by Drake).
