...
**Last updated:** 2026-09-07 (C1 merged as `059fa77a`, awaiting deploy; steps 1–4 of §What happens next are closed, step 5 waits on the sign-off line)
...
- **Nothing brings a stranger to the signup page.** The landing page is now honest (C1,
  merged 2026-09-07 — real captures, founder note, no filler stats) but honest is not the
  same as found: there is no channel, no widget, no outreach. That is step 6.
...
4. ~~**Landing-page credibility**~~ (`IMPROVEMENT_SESSIONS.md` C1) — **merged 2026-09-07
   00:16 UTC (`059fa77a`), on prod with the next deploy.** Trust bar out, founder note under the hero, real captures of the app
...
5. **The three spine features, one session each**, after §The decision carries a name.
   **This is now the head of the queue and it is blocked on the sign-off line, not on
   code.** Nothing else in this list is a session. Two owner tasks sit beside it: deploy
   `main` so C1 reaches prod, and read the landing copy once (the switching section
   promises "send your customer list through the contact form" — a manual import by Drake).
