...
| 4 | S14 · Landing: real product imagery instead of the fake mock | **DONE** 2026-09-06 as part of C1 (PR #250) — four captures of the real app, regenerated by `scripts/landing_shots.py` from a seeded demo shop; see S14 |
| 4 | S15 · Landing: trust bar rewrite | **DONE** 2026-09-06 as part of C1 (PR #250) — the bar is out, four checkable claims + the founder's note under the hero; see S15 |
...
**Done 2026-09-06 (PR #250, with S15 and the rest of `IMPROVEMENT_SESSIONS.md` C1).**
`python scripts/landing_shots.py` is the command: it migrates a throwaway SQLite DB, builds
the fictional Clearview Auto Glass shop with `manage.py seed_demo_shop` (deterministic,
~60 jobs over 60 days, invoices in every state, a portal contact with work awaiting
approval), starts the dev server, drives the installed Chrome headless over the DevTools
protocol (tornado's websocket client — no new dependency; the S10 trick), logs in with a
real session cookie, and writes four WebP captures to `static/images/landing/`: the owner
dashboard (90-day revenue view), the jobs list, the New Job form on a phone, and the
customer portal on a phone. The hero shows the dashboard in a browser frame with the phone
overlapping it; a "This is the actual product" section shows the other two. The
4-frame scroll-scrubbed sequence idea was not built — a still that is honest beats motion
that is not, and S16 is parked. `tests/test_landing_credibility.py` fails if a capture is
missing, unreferenced, lacks dimensions/alt, or if any of the mock's hardcoded numbers
come back. **Re-run the script after any visible change to those four pages and commit
the output.**
...
**Done 2026-09-06 (PR #250).** The bar is gone. Under the hero shot sit four checkable
claims: card payments processed by Stripe, works in the browser on any phone with nothing
to install, built and run by a working windshield repair shop in Arkansas, no contract.
"Export your data anytime" from the brief was **not** used — the only export today is the
AR aging CSV, and that claim would have been the new filler. The founder's note moved from
below the features to a band directly under the hero, labelled "A note from the founder",
rewritten in the first person with the 2018 date and the fact that he answers the support
form. The hero pill reads "Built by a glass shop owner in Arkansas". Two overclaims
elsewhere on the page went with it: "Email and text notifications" (SMS is not live) and
"See why shop owners love it". Copy is a draft for Drake's eye, as the brief said.
