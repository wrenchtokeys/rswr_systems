...
| N — The tech finds out | N2 · Fix dead verification SMS + tech texts | S | **BUILT 2026-09-17** — `send_sms` exists (both verification flows had failed 100% since launch), consent record + staff opt-in surface shipped. Waiting only on registration v5 + `SMS_ORIGINATION_IDENTITY` (`docs/operations/SMS_REGISTRATION.md`) |
...
- **N2** is built as of 2026-09-17; it waits on toll-free registration **version 5**, scoped to staff notifications (`docs/operations/SMS_REGISTRATION.md` §3.5).
...
RS Systems' toll-free number `+18663115189` is **PENDING** — **all four** registration versions were denied (opt-in language; a missing-field auto-deny; business-email domain + a "pre-selected" checkbox; and finally use-case + opt-in *mismatch*). The first three were fixable wording; the fourth is structural — but only for the audience all four were scoped to. **Texting a shop's customers as the shop cannot be registered here; texting RS Systems' own owners and techs as RS Systems can, and is version 5.** Full analysis: **`docs/operations/SMS_REGISTRATION.md`** (§3.5 is the split). All current SMS senders are customer-facing (invoice texts, review texts); nothing texts a tech. Prod is inert rather than broken — `SMS_ENABLED=true` but `SMS_ORIGINATION_IDENTITY` is unset, and `SMSService.is_enabled()` requires both.
...
3. ~~**`repair_request_submitted` still maps to `['in_app','sms']`.**~~ *(found by S5; **fixed** by `core/0033`, which set `channels_override=['in_app','email','sms']` on it — the shop does get an email today. Verified 2026-09-17.)*
...
| **N2** · Tech assignment texts | **Built 2026-09-17** | Four denials were four customer-facing submissions; **staff texts are a clean registration** and that is version 5. Transport, consent record and opt-in surface all landed; waiting on v5 + `SMS_ORIGINATION_IDENTITY` — `docs/operations/SMS_REGISTRATION.md` §3.5. |
...
consent surface, the consent record and `SMSService.send_sms` all landed; what remains is filing
version 5 and flipping `SMS_ORIGINATION_IDENTITY`. N4 is closed: its opt-in product work was
correct and is reused, its registration goal was misscoped.
...
| 2026-09-17 | **Corrected yesterday's SMS verdict and built the staff path (N2).** The 09-16 conclusion — *"the number is unregisterable, release it, no version 5"* — rested on *"RS Systems never texts anyone as itself,"* which is **false**: `repair_request_submitted`, `repair_assigned`, `repair_approved`, `repair_denied`, `batch_approved` and the phone-verification codes all go to **RS Systems' own account holders**, and the verification text has always read *"Your RS Systems verification code is…"*. All four denied versions were scoped as customer-facing texts sent on behalf of a shop; **staff notifications are a different, ordinary registration** (`ACCOUNT_NOTIFICATIONS`, registrant brand == message brand, consent on a logged-in page, non-circular) and that is version 5. Number kept. Built: **`SMSService.send_sms`**, which two shipped verification flows had been calling since 2026-08-09 without it existing — `AttributeError` into a bare `except`, **100% failure in production**, and both callers also gated on `settings.SMS_ENABLED` rather than `is_enabled()`, so they reported success while nothing sent; a **consent record** (`sms_consent_at`/`sms_consent_source`, `core/0035`, idempotent) that `can_send_sms()` now requires alongside switch and verified phone, with `_queue_delivery` routed through it so an owner ticking a tech's box is not consent; and the **compliant consent block** at Settings → Notifications that is version 5's screenshot surface, pinned by `tests/test_sms_consent_surface.py` (which asserts the checkbox is unchecked **in the rendered HTML**, because a screenshot of rendered HTML is what v3 was denied for). Also retired a stale N3 note: `repair_request_submitted` was fixed by `core/0033` and does get an email today. **Unrelated flake found:** `tests.test_notification_surfaces.NotificationListTests.test_day_headers_group_the_list` builds rows at `now − 1h`/`now − 2h` and asserts one "Today" header, so it fails for anyone running the suite between midnight and ~02:00 local — it is not in the baseline, so the guard script blames the running session. |
