...
| S — Where and when | S8 · Per-tenant timezone | M | TODO (not urgent — harmless until a shop outside America/Chicago signs up, then it is a correctness bug; decision taken: infer from ZIP, editable) |
...
| **Decisions needed** | **Taken 2026-08-18 (Drake): inferred.** Derive the timezone from the shop's business address once, at signup, and leave it editable in Owner Settings → General. An owner should never have to know what "America/Chicago" means. Implementation note so this isn't re-thought cold: **infer from ZIP, not state** — Kansas, Nebraska, Texas, Florida, Tennessee, Michigan and the Dakotas all span two zones, so a state map is wrong for real shops. A static ZIP-prefix → zone table covers the US in a few dozen lines with no dependency and no network call; fall back to `America/Chicago` (today's global default) when the ZIP is missing or unrecognised, which makes the inference strictly better than the status quo and never worse. Whatever is inferred is a *default*, not a lock — the Settings field is the source of truth once an owner touches it. |
